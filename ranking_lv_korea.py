...
# 戻れるのは1日分だけ
for sn in range(0,4):
    url = 'http://redstone.logickorea.co.kr/userboard/rankboard/default.aspx?boardType=5&server=' +str(sn)
    ...
    with open('out/korea_lv/' + t_server + ".csv", "w", encoding='utf-8') as f:
        for r in mat:
            f.write(','.join(r))
            f.write('\n')
    ...
    last_year_file = 'out/korea_lv/'  + tl_server + ".csv"
