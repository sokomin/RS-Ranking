const server_name = ["0","1","2","3"];

//CSVファイルを読み込む
function getCSV(date1, date2, server, job) {
    url1 = "https://sokomin.github.io/RS-Ranking/out/korea_detective/" + date1 + "_" + server_name[server] + ".csv";
    url2 = "https://sokomin.github.io/RS-Ranking/out/korea_detective/" + date2 + "_" + server_name[server] + ".csv";
...
var obj_format = {0: "rank", 1: "img", 2:"name", 3:"lv", 4:"dpt"};
var rank_now = {};
var rank_comp = {};
...
function createTable() {
    ...
    var $th_info = $('<tr>');
    var quest_title = "<th>順位</th><th>名前</th><th>レベル</th><th>ポイント</th><th>差分</th>"
    ...
    for (var i in rank_now) {
        ...
        var diff = "";
        if(comp_data){
            var diff_lv = Number(data.dpt) - Number(comp_data.dpt);
            if(diff_lv > 0){
                diff = '<font color="red">+'+diff_lv+'pt</font>';
            } else if (diff_lv < 0){
                diff = '<font color="blue">-'+diff_lv+'pt</font>';
            } else {
        ...
        var $rp = $('<tr>');//{0: "rank", 1: "img", 2:"name", 3:"lv", 4:"returner"};
        var user_data = "<td>"+ data["rank"] +"</td><td>"+data["name"]+"</td><td>"+data["lv"]+"</td><td>"+data["dpt"]+"</td><td>"+diff+"</td>";
        $rp.append(user_data);
        $table.append($rp);
        cnt++;
    }
